...
function itemToLi(item: OrderConfirmationEmailData['items'][number]) {
  ...
  return (
    <li>
      <b>{productName}</b>
      {variantName && ' (' + variantName + '), '} - Qty: {item.quantity},{' '}
      <b>
        $
        {((item.quantity * priceUSD) / 100).toLocaleString('en-US', {
...
function getAccountOrdersLink() {
  return `${process.env.PAYLOAD_PUBLIC_SERVER_URL}/orders`
}

function formatShippingAddress(address: OrderConfirmationEmailData['shippingAddress']) {
  const { firstName, lastName, addressLine1, addressLine2, city, state, postalCode, country } =
    address
  ...
  return (
    <Html>
      ...
      >
        <Img
          className="w-full"
          src="https://lepi-labs.com/api/media/file/bluesky-header-1.png?"
          alt="Lepi Labs logo"
        />
        <h1 className="text-center">Thanks for your order!</h1>
        <hr />
        <p>
          Hey there, {data.customerName}! Here&apos;s the receipt for your online order (
          {data.orderId}), placed on{' '}
          <time dateTime={data.orderCreatedAt}>
            {formatDateTime({ date: data.orderCreatedAt, format: 'MMMM dd, yyyy' })}
          </time>
          .
        </p>
        <ul>{data.items.map(itemToLi)}</ul>
        <p>
          Subtotal:{' '}
          <b>
            $
        ...
        <p>
          ...
          , {data.shippingRate.displayName}
        </p>
        <p>
          Total:{' '}
          <b>
            $
            {(data.subtotal / 100).toLocaleString('en-US', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </b>
        ...
        <hr />
        <p>
          You can view your order in your <a href={getAccountOrdersLink()}>account</a>. Another
          email with tracking information will be sent once your order has shipped. If you have any
          questions or issues, please let us know through any of our social media platforms, or by
          sending an inquiry to me at <a href="mailto:xenu@lepi-labs.com">xenu@lepi-labs.com</a>.
        </p>
        <p>Thanks!</p>
      </Tailwind>
    </Html>
...
OrderConfirmationEmail.PreviewProps = {
  ...
  items: [
    {
      product: { id: '1', title: 'Blue Widget', priceInUSD: 25.0 } as Product,
      variant: { id: 'v1', title: 'Large', priceInUSD: 0 } as Variant,
      quantity: 1,
    },
    {
      product: { id: '2', title: 'Red Gadget', priceInUSD: 10.0 } as Product,
      variant: null,
      quantity: 2,
    },
  ],
  subtotal: 35.0,
  shippingRate: { cost: 5.0, displayName: 'Standard Shipping' } as ShippingRateJSON,
  shippingAddress: {
    firstName: 'Jane',
    lastName: 'Doe',
